  ...
  if (!user) {
    ...
    if (referred_by && referred_by !== telegram_id) {
      ...
      if (referrer) {
        ...
        if (bot) bot.sendMessage(referrer.telegram_id, `🎉 ጓደኛዎ ተቀላቅለዋል! ${REFERRAL_BONUS} ብር ቦነስ አግኝተዋል።`).catch(()=>{});
      }
    }
...
if (BOT_TOKEN) {
  ...
  bot.onText(/\/start(?: (.*))?/, (msg, match) => {
    ...
    bot.sendMessage(chatId, 
      `እንኳን ደህና መጡ! ${SIGNUP_BONUS} ብር ቦነስ አግኝተዋል።\n\n🎁 ጓደኛዎን ይጋብዙ እና ${REFERRAL_BONUS} ብር ያግኙ!\nሊንክ: ${refLink}`,
      { reply_markup: { inline_keyboard: [[ { text: '🎮 ጨዋታውን ይጫወቱ', web_app: { url: WEB_APP_URL } } ]] } }
    );
  });
...
function approveDeposit(id) {
  ...
  return { telegram_id: d.telegram_id, amount: d.amount, balance_real: db.prepare('SELECT balance_real FROM users WHERE telegram_id = ?').get(d.telegram_id).balance_real };
}
